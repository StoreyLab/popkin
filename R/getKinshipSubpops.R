...
#'
#' @export
getKinshipSubpops <- function(X, subpops, m=NA, lociOnCols=FALSE, lowMem=FALSE, verbose=FALSE) {
    ## wrapper around getA combined with subpopulation-based estimation of A_Emin
    if (verbose) message('Making A...')
    A <- getA(X, n=length(subpops), m=m, lociOnCols=lociOnCols, lowMem=lowMem)
