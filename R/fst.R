...
fst <- function(x, w=NULL) {
    ## validate inputs
    if (missing(x)) {
        stop('Fatal: you must provide a kinship matrix or vector of inbreeding coefficients!')
    }
    ...
    if (is.null(w)) {
        ...
        if (length(w) != length(x)) {
            ...
            stop('Fatal: the number of individuals differs between inbreeding vector (', length(x), ') and weight vector (', length(w), ')')
        } else {
